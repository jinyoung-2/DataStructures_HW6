...
int insertFirst(headNode* h, int key) {
	...
	node->key = key; //node에 key값 대입z

	node->link = h->first;  //node의 link에 헤드노드의 first 대입 ----------->여기 check!(주석)
	h->first = node;  //head노드를 node로 변경  Q. 이중포인터야? 아니면 단일포인터야? h가?
...
/* Q. insertNode함수를 수행할 떄, 입력받은 key값보다 큰 값이 없는 경우에는 마지막에 노드를 추가하면 되나요?
    A.*/
...
int insertNode(headNode* h, int key) 
{
	...
	if((key>=(prev->key))&&(key<=(cur->key)))		//Q. 등호 맞나?
	{
		...
		node->key=key;		//key값을 node의 데이터(key)에 대입 
	}
...
int insertLast(headNode* h, int key) 
{
	listNode* node = (listNode*)malloc(sizeof(listNode));  //key에 대한 노드 생성을 위한 동적할당 
	...
	{
		cur->link=node;  //마지막 노드의 link에 생성한 node 연결(주소 대입)
		node->key=key;	//해당 key 대입
		node->link=NULL;  //추가한 노드가 마지막에 위치하므로, 리스트의 끝을 알리고자 노드의 link에 NULL 대입
	}
