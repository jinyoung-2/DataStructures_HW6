...
int freeList(headNode* h){
	...
	free(h);	//헤드노드 메모리 해제   //Q. 이거까지 해주는 이유는 뭘까?
	return 0;
}
...
/*
연결리스트 내용
노드의 구조는 데이터 필드와 다음 노드를 가리키는 link(포인터)로 이루어져 있다. 

노드 삽입, 노드 삭제, 노드 역순으로 재배치 함수를 작성할 때, 모두 노드포인터들을 이용하여 다음 노드로 이동한다. 
그리고 리스트에 노드가 전혀 존재하지 않을 때, 노드가 1개만 존재할 때, 노드가 2개 이상 존재할 때를 모두 고려해야 한다.

노드를 역순으로 재배치하는 함수의 경우, 다른 함수들과 달리 노드포인터를 3개 이용해야 한다. 노드포인터 2개만 선언시 연속적으로 역순으로 재배치 불가능하기 때문이다. 
예를 들어 prev와 cur 노드 포인터가 있다고 해보자. cur->link=prev로 하면, 기존에 cur->link가 가리키는 노드의 연결을 제거하여 연속적으로 연속으로 재배치가 불가능하다.
